...
def index(request,app,model_name):
    ...
    return render(request,"releasemgr/index.html",locals())
...
def table_change(request,app,model_name,obj_id):
    ...
    return render(request,"releasemgr/table_change.html",locals())
...
def table_add(request,app,model_name):
    ...
    if request.method == "GET":
        ...
        if form_obj.is_valid():
            form_obj.save()
            return  redirect("/kingadmin/releasemgr/%s" % (model_name))
    return render(request,"releasemgr/table_add.html",locals())