...
sys.path.append('/usr/lib/python2.6/site-packages/redis-2.10.5-py2.6.egg')
import argparse
...
class AnsibleApi2(object):
    def __init__(self,inventory_dict,tag):
        ...
        for group_name, host_list in inventory_dict["groups"].items():
            ...
            if inventory_dict["group_variables"].get(group_name):
                for k,v in inventory_dict["group_variables"][group_name].items():
                    temp_group.set_variable(k,v)
...
class Utils(object):
    ...
    def md5check_for_config(file_path):
        try:
            ...
            cmd = '''md5sum %s/md5check.txt >  %s/.hmd5c.txt''' % file_path
            print cmd
            return_code = subprocess.call(cmd, shell=True)
            if return_code != 0:
...
class DeployHandler(object):
    ...
    def __init__(self,**kwargs):
        ...
        self.api_url = "http://10.12.10.62:8000/api/index/"
        self.headers = {"api-key":"bd2c6e15-e240-4d7f-adb0-d802593ffc29"}
        self.redis_host = "10.12.10.62"
        ...
        try:
            ...
            self.project = self.config_dic['app_prefix']
            self.version = self.config_dic['version']
            self.deploy_type = self.config_dic['deploy_type']
            self.ret = ret
    ...
    def check_app(self):
        ...
        if not self.depapps == 'all':
            ...
            for app in app_list:
                if not app in self.webapps.keys():
                    if not app in self.javaapps.keys():
    ...
    def sync_javaapps(self,app_name):
        ...
        if self.deploy_type!='uat':
            ...
            if self.src_job_dir and os.path.exists(self.src_job_dir):
                ...
                # app_config_builds_path = os.path.join(os.path.join(self.builds_dir, self.app_full_config_name), app_name)
                src_role_config_path = os.path.join(self.src_job_dir,
                                                    self.config_dic['java_app_template_path'], app_name)
    ...
    def sync_webapps(self,web_name):
        ...
        if self.deploy_type=='fun':
            ...
            fun_role_file_path = os.path.join(self.config_dic['fun_job_name'], os.path.join(self.config_dic['web_app_file_path'], web_name))
            ...
            fun_config_builds_path = os.path.join(self.config_dic['fun_job_name'], self.config_dic['web_app_template_path'], web_name)
        ...
        if self.deploy_type != 'uat':
            ...
            if self.src_job_dir and os.path.exists(self.src_job_dir):
                ...
                if not src_role_file_path.endswith('/'):
                    src_role_file_path += '/'
                app_role_file_path = os.path.join(self.job_name,
                                                  os.path.join(self.config_dic['web_app_file_path'], web_name))
    ...
    def sync_file_and_config(self):
        if self.job_type not in ['mnmd5check','promd5check']:
            ...
            #         msg = traceback.format_exc()
            #         sys.exit(msg)
            with open(os.path.join(self.last_fun_successful_dir,'change'),'w') as f:
    ...
    def sync2pro(self):
        ...
        if not os.path.exists( self.config_dic.has_key("production_mount_point")):
        ...
        try:
            ...
            #
            # with open( os.path.join(product_app_dir,"changeall") ,'w') as f:
            #     for app in app_list:
            #         f.write("%s%s" % (app,os.linesep) )

            with open(os.path.join(product_app_online_dir, "changeall"), 'w') as f:
                for app in app_list:
                    f.write("%s%s" % (app, os.linesep))

            with open(os.path.join(product_app_online_dir, ".current_version"), 'w') as f:
                f.write(self.app_full_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--job_id', type=str,  help='input the job id')
