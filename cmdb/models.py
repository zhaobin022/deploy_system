...
class Environment(models.Model):
    ...
    app_variables = models.ManyToManyField("AppVariables",related_name="en_app_var_set",blank=True)

    db_variables = models.ManyToManyField("DbVariables",related_name="en_db_var_set",blank=True,verbose_name="数据库变量")
...
class DbVariables(models.Model):
    key = models.CharField(max_length=255,validators=[varildate_var],unique=True,verbose_name="数据库key")
...
class Project(models.Model):
    ...
    version = models.ForeignKey("Version",null=True,blank=True,related_name="version")
...
class HostEnvironmentRelation(models.Model):
    ...
    app_foot = models.ForeignKey(JavaAppFoot,blank=True,null=True)
    ...
    tomcat = models.ForeignKey(Tomcat,blank=True,null=True)
...
class DbBaseInfo(models.Model):
    ...
    host = models.ForeignKey(DbVariables,related_name="host_set")
    port = models.ForeignKey(DbVariables,related_name="post_set")
    sid = models.ForeignKey(DbVariables,related_name="sid_set")
    db_user = models.ForeignKey(DbVariables,related_name="db_user_set")
    db_password = models.ForeignKey(DbVariables,related_name="db_password_set")
...
class ExecuteSqlLog(models.Model):
    db = models.ForeignKey(DbBaseInfo)
    svn = models.ForeignKey(SvnPath)
