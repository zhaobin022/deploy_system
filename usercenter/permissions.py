...
def check_permission_for_spical_url(user,url,method):
    try:
        flag = False
        resolve_url_obj = resolve(url)
        current_url_name = resolve_url_obj.url_name
        for permission_key , permission_list in perm_dic.items():
            url_name = permission_list[0]
            method_list = permission_list[1]


            if url_name == current_url_name and method in method_list:
                temp = permission_key.split('_', 1)
                permission_key = (".").join(temp)
                if user.has_perm(permission_key):
                    flag = True
                    break
...
def perm_check(*args,**kwargs):
    ...
    for permission_key,permission_val in  perm_dic.items():
        ...
        per_method_list  = permission_val[1]
        ...
        if per_url_name == current_url_name: #matches current request url
            if  request.method in per_method_list: #matches request method
    ...
    try:
        ...
        if match_results and all(match_results):
            # print match_key,11111
            app_name,permission = match_key.split('_',1)
            # print("--->matched ",match_results,match_key)
            # print(app_name, *per_name)
            perm_obj = '%s.%s' % (app_name,permission)
            ...
            if request.user.has_perm(perm_obj):
                ...
                print('current user don"t have the permisssion')
                return False
